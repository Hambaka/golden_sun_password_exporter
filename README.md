...
选项：
  ...
  -e, --export
          将存档数据导出为文本文件，可搭配 Dyrati 的“黄金太阳密码生成器”使用
...
- `grade` 为**可选选项**，不使用时会自动指定默认值`g`，若使用则需要手动指定值。
- `all` 为**可选选项**，若不使用只会导出通关存档的数据，若使用则会导出全部存档的数据。
- `text` 为**可选选项**，若使用则需要手动指定值。
- `memory` 为**可选选项**。
- `cheat` 为**可选选项**，若使用则需要手动指定值。
- `export` 为**可选选项**。
...
- `grade` 为**可选选项**，若指定的级别高于输入文件的级别，则程序退出，若级别相同则忽略，若可降级则生成降级后的密码文本文件。若没有使用 `text` 选项，输出的密码版本同输入文件。
...
- `grade` 为**可选选项**，若指定的级别高于输入文件的级别，则程序退出，若级别相同则忽略，若可降级则生成降级后的内存转储二进制文件。
- `text` 为**可选选项**，若使用则需要手动指定值。
- `cheat` 为**可选选项**，若使用则需要手动指定值。
- `export` 为**可选选项**。
